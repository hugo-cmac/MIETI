...
static void usage(void) {
   printf("usage: hostMusicMIB [-D<tokens>] [-f] [-L] [-M] [-H] [LISTENING ADDRESSES]\n"
...
void alarmHandler(int sig){
  int status = -1;
	if (fork() == 0) {
			if (execlp("python3", "python3", "src/musicFinder.py", NULL)<0) {
...
int main (int argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv, "D:fHLM-:x:")) != EOF)
    ...
    case 'H':
      ...
      init_agent("hostMusicMIB");        /* register our .conf handlers */
      init_hostMusicMIB();  
      init_snmp("hostMusicMIB");
  ...
  init_agent("hostMusicMIB");

  /* init hostMusicMIB mib code */
  init_hostMusicMIB(); 
  init_hmAlbumTable();
