...
public class packetReader {
    ...
    public int[] getCoordAlert(){
        int coord[] = {
                ((int)(packet[14] & 0xff)) + ((int)(packet[15] & 0xff)<<8),
                ((int)(packet[16] & 0xff)) + ((int)(packet[17] & 0xff)<<8)
        };
        return coord;
    }
    //service info hello packet
    public int getLastPriority(){
        int coordx = ((int)(packet[4] & 0xff)) + ((int)(packet[5] & 0xff)<<8);
        return coordx;
    }
    public int getCarID(){
        int hash = ((int)(packet[8] & 0xff))      + ((int)(packet[9] & 0xff)<<8) +
                ((int)(packet[10] & 0xff)<<16)  + ((int)(packet[11] & 0xff)<<24);
        return hash;
    }
    public int[] getCoordService(){
        int coord[] = {
                ((int)(packet[12] & 0xff)) + ((int)(packet[13] & 0xff)<<8),
                ((int)(packet[14] & 0xff)) + ((int)(packet[15] & 0xff)<<8)
        };
        return coord;
    }
